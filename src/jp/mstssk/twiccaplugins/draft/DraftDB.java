...
public class DraftDB extends SQLiteOpenHelper {
	...
	private static final String CREATE_TABLE = "create table " + TABLE_NAME
			+ " ( " + COLUMN_ID + " integer primary key, " + COLUMN_TWEET
			+ " text, " + COLUMN_IN_REPLY_TO_STATUS_ID + " text, "
	 ...
	 * @return row id
	 */
	public long saveTweet(String tweet, String in_reply_to_status_id,
	 ...
	 * @return row id
	 */
	public long saveTweet(long id, String tweet, String in_reply_to_status_id,
			String latitude, String longitude) {
		...
		value.put(COLUMN_LONGITUDE, longitude);
		SQLiteDatabase db = getWritableDatabase();
		return db.update(TABLE_NAME, value, COLUMN_ID + "=?",
				new String[] { Long.toString(id) });
	}

	public long saveTweet(Tweet tweet) {
		return saveTweet(tweet.tweet, tweet.in_reply_to_status_id,
				tweet.latitude, tweet.longitude);
	}

	public long saveTweet(long id, Tweet tweet) {
		return saveTweet(id, tweet.tweet, tweet.in_reply_to_status_id,
				tweet.latitude, tweet.longitude);
	}
	...
	public SQLiteCursor loadTweet(Long id) {
		...
		return (SQLiteCursor) db.query(TABLE_NAME, new String[] { COLUMN_ID,
				COLUMN_TWEET, COLUMN_IN_REPLY_TO_STATUS_ID, COLUMN_LATITUDE,
				COLUMN_LONGITUDE }, selection, null, null, null, null);
	}
	...
	public Tweet loadTweet(long id) {
		...
		int index_in_reply_to_status_id = cursor
		...
		cursor.moveToFirst();
		return new Tweet(cursor.getLong(index_rowid), cursor
				.getString(index_tweet), cursor
				.getString(index_in_reply_to_status_id), cursor
				.getString(index_latitude), cursor.getString(index_longitude));
	}

	public long deleteTweet(long id) {
		SQLiteDatabase db = getWritableDatabase();
		return db.delete(TABLE_NAME, COLUMN_ID + "=?", new String[] { Long
				.toString(id) });
	}

	public int getTweetCount() {
		SQLiteDatabase db = getReadableDatabase();
		return db.query(TABLE_NAME, new String[] { COLUMN_ID }, null, null,
